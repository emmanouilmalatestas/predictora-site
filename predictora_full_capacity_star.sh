...
# 5. Certification check with retry logic
...
while true; do
  ...
  if echo "$CERT_JSON" | jq -e '."Predictora Certification".status' >/dev/null 2>&1; then
    break
  fi

  RETRY=$((RETRY+1))
  if [ "$RETRY" -ge "$MAX_RETRIES" ]; then
    log "ERROR: Backend did not return certification JSON. Aborting."
    exit 1
  fi

  log "Backend not ready yet... retrying ($RETRY/$MAX_RETRIES)"
  sleep 2
done
